...
from eval.eval_cos import val_cos


def test(dataset, gpu_id, model_path, datapath, save_root_path, group_size, img_size, img_dir_name):
    ...
    with torch.no_grad():
        ...
        ave_p, ave_j = val_cos(net, val_datapath, device, group_size=5, img_size=224, img_dir_name='image',
                                        gt_dir_name='groundtruth', img_ext=['.jpg'],
                                        gt_ext=[gt_ext[dataset]])

        print('{}  p: {:.4f}, j: {:.4f}'.format(dataset, ave_p[0], ave_j[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='./save_models/unitr_cos_swin.pth')
    parser.add_argument('--data_path', default='./datasets/test/')
    parser.add_argument('--datasets', default=['iCoseg', 'Internet', 'PASCAL_VOC'])
    ...
    gt_ext = {'iCoseg': '.png', 'Internet': '.jpg', 'PASCAL_VOC': '.png'}

    for dataset in args.datasets:
        val_datapath = [args.data_path + dataset]
        save_root_path = [args.output_dir + 'cos/' + dataset]
        print(save_root_path)
        test(dataset, gpu_id, model_path, val_datapath, save_root_path, 5, 224, 'image')
