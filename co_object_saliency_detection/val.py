...
def validation(net, datapath, device, group_size=5, img_size=224, img_dir_name='image', gt_dir_name='groundtruth',
               img_ext=['.jpg', '.jpg', '.jpg', '.jpg'], gt_ext=['.png', '.png', '.png', '.png']):
        ...
        for p in range(len(datapath)):
            ...
            for i in range(len(image_list)):
                ...
                for q in range(cur_class_mask.size(0)):
                    single_mae = calc_mae(cur_class_mask[q, :, :].numpy(), cur_class_gt[q, :, :].numpy())
                    all_mae.append(single_mae)

            dataset_mae = np.mean(all_mae)
            ave_mae.append(dataset_mae)
