...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP.\n'
            .format(self.dataset, self.method, mae, max_f, mean_f, max_e,
                    mean_e, s, avg_p))
        return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP.'.format(
            time.time() - start_time, self.dataset, self.method, mae, max_f,
            mean_f, max_e, mean_e, s, avg_p)
